...
// API endpoint — api.github.com by default; GITHUB_API_BASE (an http(s) origin)
// overrides it so a test can point the client at a local stub.  Resolved once
// at load; each process (incl. a spawned test server) reads its own env.
const API = (() => {
  const b = process.env.GITHUB_API_BASE;
  if (!b) return { mod: https, hostname: 'api.github.com', port: 443 };
  const u = new URL(b);
  return { mod: u.protocol === 'https:' ? https : http, hostname: u.hostname,
...
function apiSend(method, apiPath, bodyObj, cb) {
  ...
  const r = API.mod.request({ method, hostname: API.hostname, port: API.port, path: apiPath, headers }, resp => {
...
function listReleases(repo, cb) {
  apiGet(`/repos/${repo}/releases?per_page=30`, (err, code, body) => {
    if (err) return cb(err);
    if (code === 404) return cb(null, []);
    if (code !== 200 || !Array.isArray(body)) return cb(new Error('github ' + code));
    cb(null, body.filter(r => !r.draft).map(releaseInfo));
  });
}
...
// Does the repo carry a native UniData contribution (a udt-callc/*.c source) at
// `ref` (a tag, branch, or sha)?  A source release with native code but no
// prebuilt binary asset is what the registry central-builds.  cb(err, bool);
// a missing tree / error resolves to false (don't build on uncertainty).
function hasNativeCode(repo, ref, cb) {
  apiGet(`/repos/${repo}/git/trees/${encodeURIComponent(ref)}?recursive=1`, (err, code, body) => {
    if (err) return cb(err);
    if (code !== 200 || !body || !Array.isArray(body.tree)) return cb(null, false);
    cb(null, body.tree.some(t => t.type === 'blob' && /(^|\/)udt-callc\/[^/]+\.c$/.test(t.path || '')));
  });
}

// Fire a workflow_dispatch on `repo`'s `workflow` (a filename like
// build-dispatch.yml or a numeric id) at `ref`, passing `inputs`.  This is how
// the registry drives the self-hosted-runner build without touching Docker.
// Needs a GITHUB_TOKEN with "Actions: read and write" on the workflow's repo.
// cb(err, {ok:true}) on the 204 GitHub returns.
function dispatchWorkflow(repo, workflow, ref, inputs, cb) {
  if (!process.env.GITHUB_TOKEN) return cb(new Error('no GITHUB_TOKEN configured'));
  apiSend('POST', `/repos/${repo}/actions/workflows/${encodeURIComponent(workflow)}/dispatches`,
    { ref, inputs: inputs || {} }, (err, code, body) => {
      if (err) return cb(err);
      if (code === 204) return cb(null, { ok: true });
      cb(new Error('github ' + code + (body && body.message ? ': ' + body.message : '')));
    });
}
...
module.exports = { latestRelease, listReleases, verifyWebhook, releaseInfo, okRepo, createWebhook, deleteWebhook, parseConnect, repoFile, fetchUrl, defaultBranch, hasNativeCode, dispatchWorkflow };
