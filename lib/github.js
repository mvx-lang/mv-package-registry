...
// cb(err, [release,...]) — the repo's releases, newest first (excludes drafts).
//
// EVERY RELEASE, NOT THE FIRST PAGE.  This asked for `?per_page=30` and stopped,
// so a repo with more than 30 releases had a history that simply ended: mvpkg
// has 51, and after the per-version artifact indexing (#41) exactly the newest
// 30 of its 45 known versions had artifacts and the older 15 had none -- the
// count itself is the fingerprint of an unpaginated fetch.  Those versions then
// fell back to a guessed URL, which is what #41 exists to stop.
//
// 100 is GitHub's maximum page size, so most repos still take ONE call.  The
// walk stops at a short page, and at MAXPAGES so a repo with thousands of
// releases cannot turn one refresh into an unbounded crawl.
const RELPAGE = 100, MAXPAGES = 10;
// The page walk on its own, over a fetchPage(n, cb(err, code, body)).  Split out
// so the paging can be tested without reaching GitHub: the conditions that end
// it (a short page, a 404, the cap) are the whole point and are otherwise only
// reachable from a repo with hundreds of releases.
function walkReleasePages(fetchPage, cb) {
  const out = [];
  const page = n => fetchPage(n, (err, code, body) => {
    if (err) return cb(err);
    if (code === 404) return cb(null, out);
    if (code !== 200 || !Array.isArray(body)) return cb(new Error('github ' + code));
    for (const r of body) if (!r.draft) out.push(releaseInfo(r));
    // A SHORT PAGE IS THE LAST ONE, and the length that decides is what GitHub
    // returned -- not what survived the draft filter, or a page that happened to
    // be all drafts would end the walk with releases still unread.
    if (body.length < RELPAGE || n >= MAXPAGES) return cb(null, out);
    page(n + 1);
  });
  page(1);
}
function listReleases(repo, cb) {
  walkReleasePages((n, done) =>
    apiGet(`/repos/${repo}/releases?per_page=${RELPAGE}&page=${n}`, done), cb);
}
...
const releaseInfo = b => ({
  ...
  // download_count comes with the asset and is the only download figure anyone
  // here can have: the registry indexes rather than hosts, so a download goes
  // straight from the client to this URL and is never seen by us.
  assets: (b.assets || []).map(a => ({ name: a.name, url: a.browser_download_url,
                                       size: a.size, downloads: a.download_count || 0 })),
});
...
// The repo's default branch (for a dev version when there is no release).
function defaultBranch(repo, cb) {
  apiGet(`/repos/${repo}`, (err, code, body) => {
    if (err) return cb(err);
    if (code !== 200 || !body || !body.default_branch) return cb(new Error('github ' + code + (body && body.message ? ': ' + body.message : '')));
    cb(null, body.default_branch);
  });
}

module.exports = { latestRelease, listReleases, walkReleasePages, RELPAGE, MAXPAGES, verifyWebhook, releaseInfo, okRepo, createWebhook, deleteWebhook, parseConnect, repoFile, fetchUrl, defaultBranch };
