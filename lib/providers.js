...
const github = {
  ...
  // Central build: only a GitHub source can be built via workflow_dispatch on
  // the self-hosted runner.  hasNativeCode reads the source repo at a ref;
  // dispatchBuild fires the central build workflow (its repo, not the package's).
  buildable: true,
  hasNativeCode(ref, atRef, cb) { gh.hasNativeCode(ref.repo, atRef, cb); },
  dispatchBuild(dispatch, inputs, cb) { gh.dispatchWorkflow(dispatch.repo, dispatch.workflow, dispatch.ref, inputs, cb); },
  ...
  verifyEvent(secret, headers, body) {
    ...
    // 'edited'/'released'/'prereleased' too, not just 'published' — assets added
    // by a later CI job arrive as an edit; re-indexing is idempotent.
    if (p && p.release && ['published', 'released', 'prereleased', 'created', 'edited'].includes(p.action))
