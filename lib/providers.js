...
const github = {
  ...
  // release history, newest first: [{version, tag, at, html, assets, prerelease}, ...]
  listVersions(ref, cb) {
    gh.listReleases(ref.repo, (e, rels) => e ? cb(e)
      // assets and prerelease ride along so the registry records each version's
      // own artifacts rather than guessing a URL for it (#41)
      : cb(null, (rels || []).map(r => ({ version: String(r.tag || '').replace(/^v/, ''), tag: r.tag,
                                          at: r.at, html: r.html, assets: r.assets || [],
